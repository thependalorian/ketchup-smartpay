 ...
 */
router.post('/payments', requireScope('banking:payments.write'), async (req, res) => {
    try {
        const oauth = req.oauth;
        const { debtorAccountId, paymentType, creditorName, creditorAccount, creditorBankCode, amount, currency, reference, description, instructionId, } = req.body;
        ...
        const payment = await pisService.makePayment(oauth.participantId, oauth.beneficiaryId, debtorAccountId, paymentType, creditorName, creditorAccount, amount, currency || 'NAD', creditorBankCode, reference, description, instructionId);
 ...
 */
router.get('/payments/:paymentId', requireScope('banking:payments.read'), async (req, res) => {
    try {
        const oauth = req.oauth;
 ...
 * Required Scope: banking:payments.write
 */
router.get('/accounts/:accountId/beneficiaries', requireScope('banking:payments.write'), async (req, res) => {
    try {
