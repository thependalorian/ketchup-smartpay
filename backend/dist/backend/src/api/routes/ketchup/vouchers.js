...
import { NAMIBIAN_REGIONS } from '../../../../../shared/types';
...
router.get('/', async (req, res) => {
    try {
        ...
        const validStatuses = ['issued', 'delivered', 'redeemed', 'expired', 'cancelled', 'failed'];
        const filters = {
            ...(beneficiaryId != null && beneficiaryId !== '' && { beneficiaryId }),
            ...(region != null && region !== '' && NAMIBIAN_REGIONS.includes(region) && { region: region }),
            ...(grantType != null && grantType !== '' && { grantType }),
            ...(status != null && status !== '' && validStatuses.includes(status) && { status: status }),
