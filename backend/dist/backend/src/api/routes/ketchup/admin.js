...
router.get('/smartpay/health', async (_req, res) => {
    try {
        res.json({
            success: true,
            data: {
...
router.get('/smartpay/sync-logs', async (req, res) => {
    try {
...
router.get('/audit-logs/query', async (req, res) => {
    try {
        const limit = Math.min(parseInt(req.query.limit) || 50, 200);
        const endpointPrefix = req.query.endpoint_prefix || undefined;
        let result = [];
        try {
...
router.post('/audit-logs/retention', async (req, res) => {
    try {
...
router.get('/trust-account/status', async (_req, res) => {
    try {
...
router.post('/trust-account/reconcile', async (req, res) => {
    try {
        const reconciledBy = req.body?.reconciledBy || 'Ketchup Portal';
...
router.get('/compliance/monthly-stats', async (req, res) => {
    try {
