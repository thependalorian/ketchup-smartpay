...
const BANK_CODE_TO_PARTICIPANT = {
...
export class PaymentInitiationService {
     ...
     */
    async makePayment(participantId, beneficiaryId, debtorAccountId, paymentType, creditorName, creditorAccount, amount, currency = 'NAD', creditorBankCode, reference, description, instructionId) {
        try {
      ...
      `;
            ...
            this.processPaymentAsync(payment.paymentId).catch(err => logError('Async payment processing failed', err));
            return payment;
        }
        catch (error) {
     ...
     */
    async getPaymentStatus(paymentId, participantId) {
        try {
            const [payment] = await sql `
        SELECT 
          payment_id as "paymentId",
     ...
     */
    async listBeneficiaries(accountId) {
        try {
     ...
     */
    async notifyPaymentStatusChange(paymentId, status, endToEndId, statusReason) {
        if (!PAYMENT_STATUS_WEBHOOK_URL)
            return;
        try {
            const [payment] = await sql `
...
//# sourceMappingURL=PaymentInitiationService.js.map