...
export class MobileUnitService {
    ...
    async getById(id) {
        ...
        if (!agent || agent.type !== MOBILE_UNIT_TYPE)
            return null;
        return agent;
    }
    async getEquipment(mobileUnitId) {
        try {
    ...
    async getEquipmentTypes() {
        try {
            ...
            return rows.map((r) => ({ code: r.code, label: r.label, category: r.category }));
        }
        catch (error) {
    ...
    async issueEquipment(mobileUnitId, body) {
        const unit = await this.getById(mobileUnitId);
        if (!unit)
            throw new Error('Mobile unit not found');
        const [row] = await sql `
    ...
    async returnEquipment(mobileUnitId, equipmentId) {
        const unit = await this.getById(mobileUnitId);
        if (!unit)
            return null;
        const [row] = await sql `
    ...
    async getActivity(mobileUnitId, limit = 50) {
        const unit = await this.getById(mobileUnitId);
        if (!unit)
            return [];
        try {
    ...
    async postMaintenance(mobileUnitId, body) {
        const unit = await this.getById(mobileUnitId);
        if (!unit)
            throw new Error('Mobile unit not found');
    ...
    async addDriver(mobileUnitId, body) {
        const unit = await this.getById(mobileUnitId);
        if (!unit)
            throw new Error('Mobile unit not found');
        const [row] = await sql `
    ...
    async updateDriver(mobileUnitId, driverId, body) {
        const unit = await this.getById(mobileUnitId);
        if (!unit)
            return null;
    ...
    async removeDriver(mobileUnitId, driverId) {
        const unit = await this.getById(mobileUnitId);
        if (!unit)
            return false;
...
//# sourceMappingURL=MobileUnitService.js.map