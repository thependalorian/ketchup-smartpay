...
export class StatusMonitor {
     ...
     */
    async trackStatus(voucherId, status, metadata, triggeredBy = 'system') {
        try {
     ...
     */
    async recordBeneficiaryDeceased(beneficiaryId, fromStatus, triggeredBy = 'manual') {
        try {
     ...
     */
    async getStatusHistory(voucherId) {
        try {
            const rows = await sql `
