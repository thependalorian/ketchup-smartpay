 ...
 */
export function generateIdempotencyKey(voucherId, status, timestamp) {
...
export class IdempotencyService {
     ...
     */
    async getCachedResponse(idempotencyKey, endpointPrefix = ENDPOINT_DISTRIBUTION) {
        try {
     ...
     */
    async setCachedResponse(idempotencyKey, status, body, endpointPrefix = ENDPOINT_DISTRIBUTION) {
        try {
            await sql `
     ...
     * Returns cached response if exists and not expired.
     */
    async check(key, endpointPrefix = ENDPOINT_DISTRIBUTION) {
     ...
     */
    async store(key, status, body, endpointPrefix = ENDPOINT_DISTRIBUTION) {
