...
export class BuffrAPIClient {
     ...
     */
    async sendVoucher(voucher, enrichment) {
        try {
            ...
            if (enrichment?.beneficiaryIdNumber != null)
                body.beneficiary_id_number = enrichment.beneficiaryIdNumber;
            if (enrichment?.beneficiaryPhone != null)
                body.beneficiary_phone = enrichment.beneficiaryPhone;
            ...
            if (enrichment?.tokenId != null)
                body.token_id = enrichment.tokenId;
            if (enrichment?.token != null)
                body.token = enrichment.token;
     ...
     */
    async sendBatch(vouchers, getBeneficiary) {
        try {
            ...
            for (let i = 0; i < vouchers.length; i += concurrency) {
                ...
                const batchResults = await Promise.all(batch.map(async (voucher) => {
...
//# sourceMappingURL=BuffrAPIClient.js.map