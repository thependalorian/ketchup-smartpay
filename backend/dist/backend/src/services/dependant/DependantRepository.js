...
const DEPENDANT_RELATIONSHIPS = ['child', 'spouse', 'guardian', 'other'];
function rowToDependant(row) {
    const relationship = row.relationship != null && DEPENDANT_RELATIONSHIPS.includes(String(row.relationship))
...
export class DependantRepository {
    async findByBeneficiaryId(beneficiaryId) {
      ...
      `;
            const list = rows.map(rowToDependant);
    ...
    async findById(beneficiaryId, dependantId) {
        try {
            const rows = await sql `
        SELECT id, beneficiary_id, name, id_number, phone, relationship, date_of_birth, is_proxy, created_at, updated_at
        FROM beneficiary_dependants
    ...
    async create(beneficiaryId, id, data) {
        try {
            ...
            if (!DEPENDANT_RELATIONSHIPS.includes(relationship)) {
      ...
      `;
            ...
            if (!created)
                throw new Error('Dependant create succeeded but findById returned null');
    ...
    async update(beneficiaryId, dependantId, data) {
      ...
      `;
            ...
            if (!updated)
                throw new Error('Dependant update succeeded but findById returned null');
...
//# sourceMappingURL=DependantRepository.js.map