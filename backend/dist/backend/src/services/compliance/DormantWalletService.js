...
export class DormantWalletService {
    static DORMANCY_PERIOD_MONTHS = 6; // PSD-3 Section 11.4.1
    static NOTIFICATION_PERIOD_MONTHS = 5; // 1 month before dormancy
    static SEPARATE_ACCOUNT_PERIOD_YEARS = 3; // PSD-3 Section 11.4.5(d)
    /**
     * Identify wallets approaching dormancy (5 months inactive)
     * PSD-3 Section 11.4.2: Notify 1 month before dormancy
     */
    static async identifyApproachingDormancy() {
        try {
     ...
     */
    static async markWalletsAsDormant() {
        try {
            const sixMonthsAgo = new Date();
            sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - this.DORMANCY_PERIOD_MONTHS);
     ...
     * PSD-3 Section 11.4.2: Notify 1 month before dormancy
     */
    static async sendDormancyNotifications() {
        try {
     ...
     */
    static async processDormantFunds(dormantWalletId) {
        try {
     ...
     */
    static async getDormancyStatistics() {
        try {
            const result = await sql `
        SELECT 
     ...
     */
    static async sendNotification(wallet) {
     ...
     */
    static async runDailyDormancyCheck() {
        try {
     ...
     */
    static async getDormantWallets(status) {
        try {
            let query;
            if (status) {
                query = await sql `
...
//# sourceMappingURL=DormantWalletService.js.map