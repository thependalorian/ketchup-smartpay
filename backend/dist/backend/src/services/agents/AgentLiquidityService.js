...
export class AgentLiquidityService {
    async getFloatStatus(agentId) {
      ...
      `;
            ...
            if (r.currentFloat < 0)
                alert = 'overdraft';
            else if (r.currentFloat < r.threshold * 0.5)
                alert = 'critical';
            else if (r.currentFloat < r.threshold)
                alert = 'low';
            return { ...r, agentId: row.agentId, alert };
        }
        catch (e) {
    ...
    async requestTopUp(agentId, amount) {
        try {
...
//# sourceMappingURL=AgentLiquidityService.js.map