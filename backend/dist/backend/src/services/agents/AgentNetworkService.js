...
export class AgentNetworkService {
     ...
     * Top up agent float
     */
    async topupFloat(agentId, amount, notes, processedBy) {
        const agent = await sql `
     ...
     */
    async withdrawFloat(agentId, amount, notes, approvedBy) {
        const agent = await sql `
      SELECT id, liquidity_balance FROM agents WHERE id = ${agentId}
    `;
        if (agent.length === 0) {
     ...
     */
    async getAgentTransactions(agentId, days = 30) {
        const transactions = await sql `
      SELECT 
     ...
     */
    async getAgentPerformance(agentId, days = 30) {
     ...
     */
    async getPerformanceRankings(type, metric = 'volume', limit = 20) {
        let query = `
      SELECT 
        at.agent_id,
        a.name,
     ...
     */
    async getLowLiquidityAlerts(threshold = 1.5) {
...
//# sourceMappingURL=AgentNetworkService.js.map