...
export class VoucherRepository {
    ...
    async create(data) {
        ...
        INSERT INTO vouchers (
          ...
          ${data.amount},
          ${data.grantType},
          ${data.type ?? data.grantType ?? 'social_grant'},
          ${data.status},
          ${data.expiryDate},
          ${data.issuedAt},
     ...
     */
    async updateExpiry(id, newExpiryDate) {
        try {
            const result = await sql `
        UPDATE vouchers
     ...
     */
    async updateStatus(id, status, redemptionMethod, redeemedAt) {
        try {
            const result = await sql `
        UPDATE vouchers
        SET 
     ...
     */
    async findByBeneficiary(beneficiaryId) {
