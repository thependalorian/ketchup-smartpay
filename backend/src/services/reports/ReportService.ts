...
export class ReportService {
  ...
  async generateMonthlyReport(month: string): Promise<any> {
    try {
      ...
      const totalBeneficiaries = Number((beneficiaries[0] as any)?.total_beneficiaries) || 0;
      ...
      `;
      const deceasedBeneficiaries = Number((deceasedCount[0] as any)?.c ?? 0);
      ...
      return {
        ...
        by_region: byRegion.map((row: any) => ({
          region: row.region,
        ...
        by_grant_type: byGrantType.map((row: any) => ({
   ...
   */
  async generateVoucherDistributionReport(month: string): Promise<any> {
    try {
      ...
      return {
        report_period: month,
        generated_at: new Date().toISOString(),
        daily_breakdown: distribution.map((row: any) => ({
          date: row.date,
          vouchers_issued: Number(row.vouchers_issued),
          amount_disbursed: Number(row.amount_disbursed),
        })),
   ...
   */
  async generateRedemptionAnalyticsReport(month: string): Promise<any> {
    try {
      ...
      return {
        report_period: month,
        generated_at: new Date().toISOString(),
        by_redemption_method: analytics.map((row: any) => ({
   ...
   */
  async generateAgentNetworkReport(month: string): Promise<any> {
    try {
      ...
      const agentsSummary = await sql`
        ...
        WHERE created_at >= ${startDate}::timestamp
          AND created_at <= ${endDate}::timestamp
      `;
      const summaryRow = agentsSummary[0] as any;
