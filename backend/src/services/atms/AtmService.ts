...
export class AtmService {
  ...
  async getStats(): Promise<AtmStats> {
    const rows = await sql`
      ...
      GROUP BY status
    `;
    const stats: AtmStats = { total: 0, operational: 0, offline: 0, maintenance: 0, lowCash: 0 };
    for (const r of rows as { status: string; count: number }[]) {
      stats.total += r.count;
      if (r.status === 'operational') stats.operational = r.count;
      else if (r.status === 'offline') stats.offline = r.count;
      else if (r.status === 'maintenance') stats.maintenance = r.count;
      else if (r.status === 'low_cash') stats.lowCash = r.count;
    }
    return stats;
  }
