...
export class TokenVaultService {
  ...
  async validateToken(token: string): Promise<ValidateTokenResult> {
    ...
    try {
      ...
      const row = rows[0] as { id: string; voucher_id: string; purpose: string; expires_at: string; used_at: string | null };
      if (row.used_at) {
   ...
   */
  async resolveTokenId(tokenId: string): Promise<ValidateTokenResult> {
    try {
      const rows = await sql`
      ...
      const row = rows[0] as { voucher_id: string; purpose: string; expires_at: string; used_at: string | null };
      if (row.used_at) {
        return { valid: false, reason: 'Token already used', voucherId: row.voucher_id };
      }
      if (new Date(row.expires_at) < new Date()) {
