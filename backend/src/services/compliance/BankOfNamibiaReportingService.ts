...
import { DormantWalletService } from './DormantWalletService';
import { CapitalRequirementsService } from './CapitalRequirementsService';
...
export class BankOfNamibiaReportingService {
   ...
   */
  static async submitReportToBoN(reportId: string, submittedBy: string): Promise<void> {
    try {
   ...
   */
  static async getPendingReports(): Promise<MonthlyReport[]> {
    try {
      const result = await sql`
   ...
   * Get report history
   */
  static async getReportHistory(months: number = 12): Promise<MonthlyReport[]> {
    try {
      const result = await sql`
        SELECT * FROM bon_monthly_reports
   ...
   */
  static async exportReportForBoN(reportId: string): Promise<string> {
    try {
      const report = await sql`
        SELECT * FROM bon_monthly_reports WHERE id = ${reportId}
      `;
