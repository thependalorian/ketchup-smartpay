...
export class CapitalRequirementsService {
  private static readonly INITIAL_CAPITAL_REQUIRED = 1_500_000; // N$1.5 million (PSD-3 Section 11.5.2)

  /**
   * Calculate ongoing capital requirement
   * PSD-3 Section 11.5.3: 6-month average of outstanding e-money liabilities
   */
  static async calculateOngoingCapitalRequirement(): Promise<number> {
    try {
   ...
   */
  static async trackDailyCapital(
    ...
    try {
      ...
      const complianceStatus: 'compliant' | 'deficient' = isCompliant ? 'compliant' : 'deficient';

      // Calculate deficiency
      let deficiencyAmount: number | null = null;
   ...
   */
  static async getCapitalComplianceStatus(): Promise<{
    isCompliant: boolean;
   ...
   */
  static async generateCapitalReport(months: number = 6): Promise<CapitalReport[]> {
    try {
      const result = await sql`
        SELECT * FROM capital_requirements_tracking
        ORDER BY tracking_date DESC
