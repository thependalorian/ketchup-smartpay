...
export class TrustAccountService {
   ...
   */
  static async performDailyReconciliation(
   ...
   */
  static async getTrustAccountBalance(): Promise<TrustAccountBalance> {
    try {
   ...
   */
  static async calculateOutstandingLiabilities(): Promise<OutstandingLiabilities> {
    try {
      const result = await sql`
        SELECT 
   ...
   */
  static async resolveDeficiency(
  ...
  static async trackInterest(
    ...
    try {
      ...
      if (remainingAfterWithdrawal < liabilities.totalLiabilities) {
        throw new Error(
          `PSD-3 VIOLATION: Cannot withdraw interest. Remaining balance (N$${remainingAfterWithdrawal}) ` +
   ...
   */
  static async getPendingDeficiencies(): Promise<ReconciliationResult[]> {
    try {
      const result = await sql`
        SELECT * FROM trust_account_reconciliation
