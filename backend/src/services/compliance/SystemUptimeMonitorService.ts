...
export class SystemUptimeMonitorService {
  private static readonly UPTIME_SLA = 99.9; // 99.9% required by PSD-12
  private static readonly ALLOWED_DOWNTIME_MINUTES_PER_MONTH = 43.2; // 0.1% of 30 days

  // Critical services to monitor (PSD-12 definition)
  private static readonly CRITICAL_SERVICES: CriticalService[] = [
   ...
   */
  static async performHealthCheck(service: CriticalService): Promise<HealthCheckResult> {
   ...
   * HTTP health check
   */
  private static readonly HTTP_CHECK_TIMEOUT_MS = 5000;

  private static async httpHealthCheck(url: string): Promise<{
    status: 'up' | 'down' | 'degraded';
    ...
    return new Promise((resolve) => {
      ...
      let resolved = false;
      const safeResolve = (out: { status: 'up' | 'down' | 'degraded'; responseTimeMs: number; errorMessage?: string }) => {
        if (resolved) return;
        resolved = true;
        resolve(out);
      };
   ...
   * Database health check
   */
  private static async databaseHealthCheck(): Promise<{
    status: 'up' | 'down' | 'degraded';
    responseTimeMs: number;
    errorMessage?: string;
  }> {
   ...
   */
  static async monitorAllServices(): Promise<HealthCheckResult[]> {
   ...
   */
  static async generateDailySummary(date: Date): Promise<UptimeReport[]> {
    try {
   ...
   */
  static async calculateMonthlyUptime(
    serviceName: string,
    year: number,
    month: number
  ): Promise<{
   ...
   */
  static async getCurrentUptimeStatus(): Promise<{
    services: Array<{
      name: string;
      status: string;
      availabilityPercentage: number;
    ...
    try {
      ...
      const serviceStatus = services.map((svc: any) => {
        const summary = summaryData.find((s: any) => s.service_name === svc.service_name);
   ...
   */
  static startContinuousMonitoring(intervalMinutes: number = 5): NodeJS.Timeout {
