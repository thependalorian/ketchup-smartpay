...
export class TwoFactorAuthService {
   ...
   */
  static async validateOTP(validation: OTPValidation): Promise<TwoFactorAuthResult> {
    try {
   ...
   */
  static async verifyTransactionAuth(transactionId: string): Promise<boolean> {
    try {
      const result = await sql`
        SELECT * FROM two_factor_auth_logs
   ...
   */
  static async get2FAStatistics(startDate: Date, endDate: Date): Promise<{
