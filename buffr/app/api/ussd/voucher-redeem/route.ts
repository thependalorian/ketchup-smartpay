/**
 * USSD Voucher Redemption API
 * 
 * Location: app/api/ussd/voucher-redeem/route.ts
 * Purpose: Redeem vouchers via USSD for feature phone users
 * 
 * Compliance: PSD-12 (2FA required)
 * Integration: USSD service for voucher redemption
 */
...
interface USSDVoucherRedeemRequest {
  sessionId: string;
  phoneNumber: string;
  voucherId: string;
...
async function postHandler(req: ExpoRequest) {
  ...
  try {
    const body: USSDVoucherRedeemRequest = await req.json();
    const { sessionId, phoneNumber, voucherId, pin, redemptionMethod } = body;

    // Validate required fields
    if (!sessionId || !phoneNumber || !voucherId || !pin || !redemptionMethod) {
      return errorResponse(
        'sessionId, phoneNumber, voucherId, pin, and redemptionMethod are required',
    ...
    return successResponse(result, 'Voucher redeemed successfully via USSD');
  } catch (error: any) {
    logger.error('USSD voucher redemption error', error);
    return errorResponse(
