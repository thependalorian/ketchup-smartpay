/**
 * Open Banking API: /api/v1/analytics/channels
 * 
 * Channel analytics (Open Banking format)
 * 
 * Requires: Admin authentication
 */

import { ExpoRequest } from 'expo-router/server';
import { openBankingSecureRoute, getResponseHelpers } from '@/utils/openBankingMiddleware';
import { parsePaginationParams, OpenBankingErrorCode } from '@/utils/openBanking';
import { RATE_LIMITS } from '@/utils/secureApi';
import { query } from '@/utils/db';
import { checkAdminAuth } from '@/utils/adminAuth';
import { log } from '@/utils/logger';

/**
 * GET /api/v1/analytics/channels
 * Get channel analytics
 */
async function handleGetChannels(req: ExpoRequest) {
  const helpers = getResponseHelpers(req);
  const context = (req as any).openBanking;
  
  try {
    // Check admin authentication
    const authResult = await checkAdminAuth(req);
    if (!authResult.authorized) {
      return helpers.error(
        OpenBankingErrorCode.FORBIDDEN,
        authResult.error || 'Admin access required',
        403
      );
    }

    const { searchParams } = new URL(req.url);
    const fromDate = searchParams.get('from_date');
    const toDate = searchParams.get('to_date');
    const channel = searchParams.get('channel') as 'mobile_app' | 'ussd' | null;
    const { page, pageSize } = parsePaginationParams(req);

    let sql = `
      SELECT 
        channel, date, transaction_count, total_volume,
        unique_users, average_transaction_amount
      FROM channel_analytics
      WHERE 1=1
    `;
    const params: any[] = [];
    let paramIndex = 1;
    const conditions: string[] = [];

    if (fromDate) {
      conditions.push(`date >= $${paramIndex++}`);
      params.push(fromDate);
    }

    if (toDate) {
      conditions.push(`date <= $${paramIndex++}`);
      params.push(toDate);
    }

    if (channel) {
      conditions.push(`channel = $${paramIndex++}`);
      params.push(channel);
    }

    if (conditions.length > 0) {
      sql += ` AND ${conditions.join(' AND ')}`;
    }

    // Get total count
    const countResult = await query<{ count: string }>(
      sql.replace('SELECT channel, date', 'SELECT COUNT(*) as count'),
      params
    );
    const total = parseInt(countResult[0]?.count || '0', 10);

    // Apply pagination
    const offset = (page - 1) * pageSize;
    sql += ` ORDER BY date DESC, total_volume DESC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
    params.push(pageSize, offset);

    const results = await query<any>(sql, params);

    // Calculate channel comparison
    const channelComparison = await query<{
      channel: string;
      total_transactions: number;
      total_volume: number;
      unique_users: number;
    }>(
      `SELECT 
        channel,
        SUM(transaction_count)::INTEGER as total_transactions,
        SUM(total_volume) as total_volume,
        COUNT(DISTINCT unique_users)::INTEGER as unique_users
      FROM channel_analytics
      WHERE 1=1
      ${conditions.length > 0 ? `AND ${conditions.map((c, i) => c.replace(/\$\d+/, `$${i + 1}`)).join(' AND ')}` : ''}
      GROUP BY channel
      ORDER BY total_volume DESC`,
      params.slice(0, -2)
    );

    // Format as Open Banking
    const formattedAnalytics = results.map((r: any) => ({
      Channel: r.channel,
      Date: r.date,
      TransactionCount: r.transaction_count || 0,
      TotalVolume: parseFloat(r.total_volume?.toString() || '0'),
      UniqueUsers: r.unique_users || 0,
      AverageTransactionAmount: parseFloat(r.average_transaction_amount?.toString() || '0'),
    }));

    const comparison = channelComparison.map((c: any) => ({
      Channel: c.channel,
      TotalTransactions: c.total_transactions || 0,
      TotalVolume: parseFloat(c.total_volume?.toString() || '0'),
      UniqueUsers: c.unique_users || 0,
    }));

    return helpers.paginated(
      formattedAnalytics,
      'ChannelAnalytics',
      '/api/v1/analytics/channels',
      page,
      pageSize,
      total,
      req,
      fromDate || toDate || channel ? { fromDate, toDate, channel } : undefined,
      undefined,
      undefined,
      context?.requestId
    );
  } catch (error) {
    log.error('Error fetching channel analytics:', error);
    return helpers.error(
      OpenBankingErrorCode.SERVER_ERROR,
      'An error occurred while fetching channel analytics',
      500
    );
  }
}

export const GET = openBankingSecureRoute(
  handleGetChannels,
  {
    rateLimitConfig: RATE_LIMITS.admin,
    requireAuth: true,
    trackResponseTime: true,
  }
);
