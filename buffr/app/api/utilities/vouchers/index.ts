 ...
 * Vouchers API Route
 * 
 * Location: app/api/utilities/vouchers/index.ts
 * Purpose: API endpoints for voucher management
 * 
 * Endpoints:
 * - GET /api/utilities/vouchers - List all vouchers for authenticated user
 * 
 * Compliance: Payment System Management Act, PSD-1/PSD-3
 * Integration: NamPay, NamPost, retail partners
 * 
 * Note: Vouchers are treated as "prepaid value" or "payment instruments",
 * NOT as virtual assets (per Virtual Assets Act, 2023)
 */
...
async function getHandler(req: ExpoRequest) {
  try {
    ...
    const vouchers = await query<{
      ...
      namqr_code: string | null; // Include NamQR code
      metadata: any;
      created_at: Date;
      updated_at: Date;
    }>(
    ...
    // Map to API format (including NamQR code)
    const mappedVouchers = vouchers.map(v => ({
      ...
      namqrCode: (v as any).namqr_code || null, // Include NamQR code if available
      metadata: v.metadata,
