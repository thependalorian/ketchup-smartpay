...
async def chat(request: ChatRequest):
    """Non-streaming chat endpoint with RAG (knowledge base retrieval)."""
    try:
...
async def chat_simple(request: ChatRequest):
    """
    Simple chat endpoint without RAG (direct LLM call).
    
    Matches TypeScript backend endpoint: POST /api/chat/simple
    Provides faster responses for simple queries that don't need knowledge base.
    """
    try:
        from .agent import rag_agent, AgentDependencies
        ...
        # Create dependencies (minimal - no RAG tools)
        deps = AgentDependencies(
            session_id=session_id,
            user_id=request.user_id
        )
        ...
        # Simple system prompt (no RAG)
        simple_prompt = """You are Buffr AI Companion, a helpful financial assistant for Buffr, Namibia's digital payment platform.
...
Keep responses concise but informative. Use emojis sparingly for friendliness."""
        ...
        # Direct LLM call without RAG tools
        ...
        response_message = result.data if hasattr(result, 'data') else str(result)
        ...
        await add_message(
            ...
            metadata={"user_id": request.user_id, "simple_chat": True}
        )
        await add_message(
            session_id=session_id,
            role="assistant",
